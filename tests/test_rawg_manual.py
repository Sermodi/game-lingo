...
async def test_search_games():
    ...
    # Obtener API key
    api_key = input("Ingresa tu RAWG API key (o presiona Enter para usar variable de entorno): ").strip()
    if not api_key:
        import os
            ...
            for query in test_queries:
                try:
                    ...
                    for i, game in enumerate(result.results[:3], 1):
                        print(f"  {i}. {game['name']} (ID: {game['id']}, Rating: {game.get('rating', 'N/A')})")
        ...
        async with RAWGAPIConnector(api_key) as connector:
            ...
            for game_id, expected_name in test_games:
                try:
                    ...
                    description = details.get('description_raw', details.get('description', ''))
                    if description:
                ...
                try:
                    ...
                    if game_info:
                        ...
                        if game_info.description:
                            desc_preview = game_info.description[:150] + "..." if len(game_info.description) > 150 else game_info.description
                            print(f"   Descripción: {desc_preview}")
                    else:
                    ...
                    if query:
                        try:
                            ...
                            if game_info:
                                ...
                                print(f"   Plataformas: {', '.join(game_info.platforms)}")
                                print(f"   Géneros: {', '.join(game_info.genres)}")
                                print(f"   Rating: {game_info.rating}")
                            else:
