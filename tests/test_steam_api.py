...
class TestSteamAPI:
    ...
    async def test_search_game_api_error(self, mock_session):
        ...
        mock_response = AsyncMock()
        mock_response.status = 200
        mock_response.json = AsyncMock(return_value={"success": False, "error": "Invalid request"})
        mock_session.get.return_value.__aenter__.return_value = mock_response
    ...
    @pytest.mark.asyncio
    async def test_get_game_details_success(self, mock_session, sample_details_response):
    ...
    @pytest.mark.asyncio
    async def test_find_game_by_name_success(self, mock_session, sample_search_response, sample_details_response):
        ...
        mock_session.get.return_value.__aenter__.side_effect = [search_mock, details_mock]
