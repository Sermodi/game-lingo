...
class SteamResponse(APIResponse):
    ...
    platforms: list[str] = Field(default_factory=list, description="Plataformas soportadas")
    ...
    screenshots: list[str] = Field(default_factory=list, description="URLs de capturas de pantalla")
    ...
    price_overview: dict[str, Any] | None = Field(None, description="Información de precio")
...
class RAWGResponse(APIResponse):
    ...
    website: str | None = Field(None, description="Sitio web oficial")
    developers: list[dict[str, Any]] = Field(default_factory=list, description="Desarrolladores")
    publishers: list[dict[str, Any]] = Field(default_factory=list, description="Distribuidores")
    platforms: list[dict[str, Any]] = Field(default_factory=list, description="Plataformas")
    ...
    screenshots: list[dict[str, str]] = Field(default_factory=list, description="Capturas")
...
class TranslationAPIResponse(APIResponse):
    ...
    confidence: float | None = Field(None, ge=0, le=1, description="Confianza en la traducción")
    provider: str = Field(..., description="Proveedor de traducción")
    characters_used: int = Field(..., ge=0, description="Caracteres utilizados")
    detected_language: str | None = Field(None, description="Idioma detectado automáticamente")