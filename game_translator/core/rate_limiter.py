...
class RateLimiter:
    ...
    async def wait_if_needed(
        ...
        raise RateLimitError(f"Failed to acquire rate limit for {api_name} after {max_retries} retries")
    ...
    def _cleanup_character_window(self, limit: APIRateLimit, current_time: float) -> None:
        ...
        try:
            ...
            if "api_states" in state:
                for api_name, api_state in state["api_states"].items():
                    if api_name in self.api_limits:
                        limit = self.api_limits[api_name]
                        limit.consecutive_errors = api_state.get("consecutive_errors", 0)
