...
from ..exceptions import GameNotFoundError, GameTranslatorError, ValidationError
from ..models.game import GameInfo, Platform, TranslationResult, TranslationSource
from ..apis import SteamAPI, DeepLAPI, GoogleTranslateAPI
...
class GameDescriptionTranslator:
    ...
    def __init__(
        ...
        logger.info(f"Translation providers: {settings.get_configured_translation_providers()}")
    ...
    async def translate_game_description(
        ...
        if not game_identifier and not english_description:
            raise ValidationError("Must provide either game_identifier or english_description")
        ...
        try:
            ...
            if english_description:
                ...
                logger.info(f"Successfully translated description for: {game_identifier}")
                return result
            ...
            if not game_info:
                raise GameNotFoundError(game_identifier, str(platform) if platform else None)
        ...
        if self._should_try_steam(platform) and self.steam_api:
            try:
                ...
                if game_info and game_info.has_spanish_description():
                    logger.info(f"Found Spanish description in Steam for: {game_identifier}")
                    return game_info
            except Exception as e:
    ...
    async def _process_translation(self, game_info: GameInfo, result: TranslationResult) -> None:
        ...
        if translated_text:
            ...
            logger.info(f"Translated description using {provider} (confidence: {confidence})")
    
    async def _translate_text(
        self, text: str, result: TranslationResult
        ...
        for provider_name in providers:
            try:
                ...
                elif provider_name == "google" and self.google_api:
    ...
    def _get_translation_providers(self) -> list[str]:
        ...
        # Fallback
        fallback = "google" if self.preferred_translation_provider == "deepl" else "deepl"
    ...
    def _validate_input(self, game_identifier: str, platform: Platform | str | None) -> None:
    ...
    async def _save_to_cache(
        self, game_identifier: str, platform: Platform | None, result: TranslationResult
    ) -> None:
    ...
    def _generate_cache_key(self, game_identifier: str, platform: Platform | None) -> str:
    ...
    async def _search_steam(self, game_identifier: str, result: TranslationResult) -> GameInfo | None:
    ...
    async def _search_rawg(
        ...
        try:
            ...
            async with self.rawg_api as rawg:
                ...
                game_info = await rawg.find_game_by_name(game_identifier, exact_match=False)
                if game_info:
