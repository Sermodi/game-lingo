...
class SteamAPI:
        ...
        try:
            ...
            async with self.session.get(self.SEARCH_URL, params=params) as response:
                ...
                if "items" not in data:
                    raise APIError(f"Steam search failed: unexpected response format: {data}", "steam")
        ...
        try:
            ...
            async with self.session.get(self.DETAILS_URL, params=params) as response:
                ...
                steam_response = SteamResponse(
                    ...
                    categories=self._extract_categories(game_data.get("categories", [])),
                    genres=self._extract_genres(game_data.get("genres", [])),
                    release_date=self._extract_release_date(game_data.get("release_date", {})),
                    metacritic_score=game_data.get("metacritic", {}).get("score"),
                    price_overview=game_data.get("price_overview", {}),
                    screenshots=self._extract_screenshots(game_data.get("screenshots", [])),
                    movies=self._extract_movies(game_data.get("movies", [])),
                    language=lang_code,
                    raw_data=data,
                )
                
                logger.info(f"Successfully retrieved Steam details for: {steam_response.name}")
    ...
    def _find_best_match(self, query: str, results: List[Dict[str, Any]]) -> Dict[str, Any] | None:
        ...
        if scored_results and scored_results[0][0] > 0:
            ...
            logger.debug(f"Best match: '{best_match.get('name', '')}' with score {best_score:.1f}")
            return best_match
    ...
    def _steam_response_to_game_info(self, steam_response: SteamResponse) -> GameInfo:
        ...
        if steam_response.language == "spanish":
            ...
            detailed_desc_es = self._clean_html(steam_response.detailed_description or steam_response.about_the_game)
            short_desc_en = None
            ...
            detailed_desc_en = self._clean_html(steam_response.detailed_description or steam_response.about_the_game)
    ...
    def _extract_categories(self, categories_data: List[Dict[str, Any]]) -> List[str]:
        ...
        return [cat.get("description", "") for cat in categories_data if cat.get("description")]
    
    def _extract_genres(self, genres_data: List[Dict[str, Any]]) -> List[str]:
        """Extrae géneros del juego."""
        return [genre.get("description", "") for genre in genres_data if genre.get("description")]
    
    def _extract_release_date(self, release_data: Dict[str, Any]) -> str | None:
        """Extrae fecha de lanzamiento."""
        return release_data.get("date") if release_data.get("coming_soon") is False else None
    ...
    async def _handle_response_errors(self, response: aiohttp.ClientResponse) -> None:
        ...
        if response.status == 429:
            ...
            raise APIError(f"Steam API server error: {response.status}", "steam", response.status)
        
        elif response.status >= 400:
            text = await response.text()
            raise APIError(f"Steam API client error {response.status}: {text}", "steam", response.status)
