...
class RAWGAPIConnector:
    ...
    async def get_game_details(self, game_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            async with self.session.get(url, params=params) as response:
                ...
                logger.info(f"Retrieved RAWG details for game: {data.get('name', 'Unknown')}")
                return data
    ...
    async def find_game_by_name(self, name: str, exact_match: bool = False) -> Optional[GameInfo]:
        """
    ...
    def _convert_to_game_info(self, rawg_data: Dict[str, Any]) -> GameInfo:
        ...
        description = rawg_data.get('description_raw', '') or rawg_data.get('description', '')
    ...
    async def _handle_response_errors(self, response: aiohttp.ClientResponse) -> None:
        ...
        if response.status == 429:
            ...
            raise APIError(f"RAWG API server error: {response.status}", "rawg", response.status)
        
        elif response.status >= 400:
            text = await response.text()
            raise APIError(f"RAWG API client error {response.status}: {text}", "rawg", response.status)
...
async def search_rawg_game(query: str, api_key: Optional[str] = None) -> Optional[GameInfo]:
    """
