...
class GoogleDetectionResult:
    ...
    def __str__(self) -> str:
        ...
        return f"{self.language} ({self.confidence * 100:.1f}% confidence, {reliability})"
...
class GoogleTranslateAPIConnector:
    ...
    def get_supported_languages(self, target_language: str = "en") -> List[GoogleLanguage]:
        """
    ...
    def detect_language(self, text: str) -> GoogleDetectionResult:
        ...
        try:
            ...
            logger.info(f"Detected language: {result.language} (confidence: {result.confidence:.2f})")
            return result
            
        except Exception as e:
            if isinstance(e, (APIError, AuthenticationError, RateLimitError, ValidationError)):
                raise
            raise APIError(
                api_name="google_translate",
    ...
    def translate_text(
        ...
        try:
            ...
            logger.info(f"Translation successful (detected source: {result.detected_source_language})")
            return result
            
        except Exception as e:
            if isinstance(e, (APIError, AuthenticationError, RateLimitError, TranslationError, ValidationError)):
                raise
            raise TranslationError(
    ...
    def translate_game_description(
        ...
        try:
            ...
            logger.error(f"Failed to translate game description: {str(e)}")
            raise
...
def translate_game_description(
    ...
    with GoogleTranslateAPIConnector(api_key=api_key) as connector:
        ...
        if game is not None:
            ...
            if not desc:
                raise ValidationError(message="Game description cannot be empty", field="description")
