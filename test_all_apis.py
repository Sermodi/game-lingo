...
from game_translator.apis.rawg_api import RAWGAPIConnector
from game_translator.apis.steam_api import SteamAPI
from game_translator.apis.deepl_api import DeepLAPIConnector
from game_translator.apis.google_translate_api import GoogleTranslateAPIConnector
from game_translator.config import settings
from game_translator.exceptions import APIError, GameNotFoundError
        ...
        async with RAWGAPIConnector(settings.RAWG_API_KEY) as connector:
            for game_name in TEST_GAMES:
                ...
                try:
                    ...
                    game_info = await connector.find_game_by_name(game_name, exact_match=False)
                    ...
                    if game_info:
                        ...
                        # Buscar descripción
                        description = game_info.short_description_en or game_info.detailed_description_en or ""
                        if description:
                            # Verificar si parece estar en español
                            spanish_indicators = ['el', 'la', 'de', 'en', 'con', 'para', 'que', 'los', 'las']
                            words = description.lower().split()
                            spanish_word_count = sum(1 for word in words[:20] if word in spanish_indicators)
                            
                            if spanish_word_count > 2:
                                print(f"   [OK] Descripcion parece estar en ESPANOL")
                                desc_preview = description[:150] + "..." if len(description) > 150 else description
                                print(f"   Preview: {desc_preview}")
                            else:
                                print(f"   [AVISO] Descripcion parece estar en INGLES")
                                desc_preview = description[:150] + "..." if len(description) > 150 else description
                                print(f"   Preview: {desc_preview}")
                        else:
                            print(f"   [AVISO] No hay descripcion disponible")
                    else:
    ...
    try:
        async with SteamAPI() as steam:
            for game_name in TEST_GAMES:
                ...
                try:
                    # Buscar en Steam
                    results = await steam.search_game(game_name, language="spanish", max_results=3)
                            ...
                            if game_id:
                                ...
                                if game_info:
                                    if game_info.short_description_es:
                                        print(f"   [OK] Descripcion en ESPANOL encontrada")
                                        desc_preview = game_info.short_description_es[:150] + "..." if len(game_info.short_description_es) > 150 else game_info.short_description_es
                                        print(f"   Preview: {desc_preview}")
                                    elif game_info.short_description_en:
                                        print(f"   [AVISO] Solo descripcion en INGLES disponible")
                                        desc_preview = game_info.short_description_en[:150] + "..." if len(game_info.short_description_en) > 150 else game_info.short_description_en
                                        print(f"   Preview: {desc_preview}")
                                    else:
                                        print(f"   [AVISO] No hay descripcion disponible")
                        except Exception as e:
...
def test_google_translate_api():
    ...
    if not settings.GOOGLE_TRANSLATE_API_KEY:
        print("[AVISO] GOOGLE_TRANSLATE_API_KEY no configurada. Saltando test de Google Translate.")
        return
    ...
    try:
        ...
        print(f"[OK] Traduccion exitosa:")
        print(f"   Idioma detectado: {getattr(result, 'detected_source_language', getattr(result, 'detected_language', 'en'))}")
        print(f"   Confianza: {getattr(result, 'confidence', 'N/A')}")
        print(f"   Caracteres usados: {getattr(result, 'characters_used', 'N/A')}")
        print(f"   Texto traducido:")
        translated_text = getattr(result, 'translated_text', getattr(result, 'text', ''))
...
async def main():
    ...
    print(f"  - Steam: [OK] (siempre disponible)")
    print(f"  - DeepL: {'[OK]' if settings.DEEPL_API_KEY else '[NO]'}")
    print(f"  - Google Translate: {'[OK]' if settings.GOOGLE_TRANSLATE_API_KEY else '[NO]'}")
